// The J language is an array programming language influenced by APL. In J, operations on individual numbers (2 * 3)
//  can just as easily be applied to entire lists of numbers (2 * 3 4 5, returning 6 8 10).

// Operators in J are referred to as verbs. Verbs are either monadic (taking a single argument, such as *: 3, "3 squared") 
...
fn main() {
    let unparsed_file = std::fs::read_to_string("prog.ijs")
